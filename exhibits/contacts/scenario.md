...
| 2 | 팩스 폐기 changeset 이 스키마·데이터·UI **세 곳에서 함께 지운다** | ① 적용되어야 통과 — 지금은 **손 저작**(10 참조) |
| 3 | 롤백이 **행마다 잃은 값을 되돌린다** | ① **완주 기준** |
| 4 | 롤백 공통 게이트 | 모든 시나리오의 완주 조건 |
| 5 | 부재 엔티티를 지목한 제거 → 422 `AdapterRefused`, 이유가 표적을 이름으로 | ② 거부되어야 통과 |
| 6 | 부재 필드를 지목한 타입 변경 → 422 `AdapterRefused`, 이유가 필드를 이름으로 | ② 거부되어야 통과 (Vivarium.Stage 0.6.0 전에는 **500 구조적 크래시**를 고정하고 있었다) |
| 7 | 부재 엔티티 제거 → 422 `AdapterRefused` | ② 거부되어야 통과 (0.6.0 전에는 아무 일 없이 `Applied` 로 남았다 — 통과가 곧 결함) |
| 8 | 기존 데이터를 위반하는 제약이 그대로 착지한다 | ③ **미선언** — 결함이 아니다 |
| 9 | 개명 run 의 산출물에서 **이름 칸 3/3 이 비었음**을 자리 선언이 잡는다 (시드는 통과) | **대조군 쌍** — 아래 |
| 10 | 게시본 agent 가 "지우고 비우는" 폐기 턴을 저작하지 **못한다** | **결함 고정** — agent ≥0.2.1 소비 시 뒤집는다 |
...
**지금 단언 2 의 폐기는 에이전트가 아니라 손으로 저작된다 (단언 10).** 호스트가 라이브
스키마·데이터를 에이전트에 공급하자 agent 0.2.0 의 저작 시점 표적 검사가 이 전시물에서
처음 구동됐고, 이 턴을 저작할 수 없다는 것이 드러났다 — 데이터 패치를 필드가 **이미
지워진** 스키마에 대고 판정해서, 지우는 필드의 값을 비우는 `set { faxNumber: null }` 을
"선언되지 않은 필드에 쓰기"로 거부한다. 수정은 agent 0.2.1 소스에 있다(미게시). 롤백이
잃은 값을 되돌리는가는 stage 의 판정이므로 같은 모양의 손 저작 changeset 으로 ① 을 계속
판정하고, 결함은 단언 10 이 고정한다 — 게시본을 소비하면 10 을 뒤집고 2 를 에이전트
제안으로 되돌린다.

### ② 거부가 정답 (5·6·7) — 그리고 **두 개의 문**
...
단언 6 은 **결함을 고정하던 자리**였다. 어댑터 계약은 이 실패 모드를 명시적으로 금지
한다 — *"부재 멤버를 역참조하면 null 참조 결함이 나고, **결함은 이유가 아니다**"* —
그런데 Vivarium.Stage 0.6.0 이전 게시본은 정확히 그것(500)을 했다. 그 사실을 판정되는
자리에 적어 두고 실패 메시지에 뒤집는 방법을 남겼고, 0.6.0 범프가 그것을 빨갛게 만든
뒤 ② 문법(422 · 필드를 이름으로)으로 뒤집혔다.
...
둘 다 "거부 없이 통과한다"였지만 **같은 것이 아니었고, 그 구분이 이 축이 남기는 값**이다 — 7 은 의존이 고치자 뒤집혔고 8 은 뒤집히지 않는다.

- **7 은 결함이었다.** 아무 일도 하지 않은 것을 했다고 말하고 원장에 완료로 남겼다.
  제거는 대상이 없어도 결과가 같아 보여 조용한 성공이 가장 그럴듯한 자리다.
  0.6.0 이 이것을 닫았고 단언은 **②로 뒤집혔다** — 지금은 거부되어야 통과한다.
...
node host/smoke-destructive.ts   # 기대: smoke-destructive: 10/10 PASS
```
