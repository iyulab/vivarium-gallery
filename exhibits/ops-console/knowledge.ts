/**
 * ops-console 전시물의 KnowledgeSource — capability SHAPE + 백엔드의 facet 규칙 + 하우스 규칙.
 *
 * 라이브 스키마·데이터는 호스트가 지문과 함께 `SchemaInput`/`DataInput` 으로 공급한다
 * (host/server.ts). 이 전시물이 다른 점은 **분량**이다 — 엔티티 셋과 행 39건이 그 뷰로
 * 모델 앞에 놓인다. 그 자체가 이 전시물이 재는 축의 일부다: 규모가 커지면 모델이
 * 읽어야 하는 맥락도 함께 커지고, 그 비용은 `/agent/metrics` 가 보고한다.
 */
